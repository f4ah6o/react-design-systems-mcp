...
describe('Error Handling Integration Tests', () => {
  ...
  describe('Resource access error handling', () => {
    test('should handle invalid resource URIs', async () => {
      // This test MUST FAIL initially
      await expect(server.readResource('cloudscape://invalid-resource-type/test')).rejects.toThrow()

      await expect(server.readResource('invalid://demos/button')).rejects.toThrow()
    })

    test('should handle missing resource parameters', async () => {
      // This test MUST FAIL initially
      await expect(server.readResource('cloudscape://demos/')).rejects.toThrow()

      await expect(server.readResource('cloudscape://patterns/')).rejects.toThrow()
    })

    test('should handle resource access for non-existent items', async () => {
      // This test MUST FAIL initially
      await expect(server.readResource('cloudscape://demos/fake-component')).rejects.toThrow(
        /not found/i,
      )

      await expect(server.readResource('cloudscape://patterns/fake-pattern')).rejects.toThrow(
        /not found/i,
      )
    })
  })
