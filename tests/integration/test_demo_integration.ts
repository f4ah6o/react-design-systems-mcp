...
describe('Demo-Component Integration Tests', () => {
  ...
  describe('Demo-Component relationship validation', () => {
    ...
    test('should provide demos for components with complex properties', async () => {
      ...
      const complexComponent = Object.values(allComponents).find(component => 
        Object.keys(component.properties).length > 5
      );
      ...
      if (complexComponent) {
        ...
        if (responseData.demos.length > 0) {
          ...
          const hasComplexDemo = responseData.demos.some((demo: any) => 
            demo.code && demo.code.length > 200 // Reasonably complex demo
          );
