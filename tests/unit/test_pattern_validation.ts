...
describe('Pattern Data Validation', () => {
  ...
  describe('Pattern category consistency', () => {
    ...
    test('patterns should be distributed across categories', () => {
      const categoryDistribution = patternCategories.reduce(
        (acc, cat) => {
          ...
          const hasReference =
            ...
            pattern.examples.some(
              (ex) =>
                ...
                ex.codeSnippet?.toLowerCase().includes(component),
            )

          if (!hasReference) {
            console.warn(
              `Pattern ${pattern.id} claims to use component ${component} but doesn't reference it`,
            )
          }
        })
      })
  ...
  describe('Pattern tag validation', () => {
    ...
    test('patterns should have relevant component tags', () => {
      patterns.forEach((pattern) => {
        const hasComponentTags = pattern.components.some(
          (component) =>
            pattern.tags.includes(component) || pattern.tags.includes(component.replace('-', '')),
        )
  ...
  describe('Pattern consistency validation', () => {
    test('patterns in same category should have consistent structure', () => {
      ...
      Object.entries(categorizedPatterns).forEach(([_category, categoryPatterns]) => {
        if (categoryPatterns.length > 1) {
          const avgExamples =
            categoryPatterns.reduce((sum, p) => sum + p.examples.length, 0) /
            categoryPatterns.length
          const avgComponents =
            categoryPatterns.reduce((sum, p) => sum + p.components.length, 0) /
            categoryPatterns.length
          const avgTags =
            categoryPatterns.reduce((sum, p) => sum + p.tags.length, 0) / categoryPatterns.length
  ...
  describe('Documentation completeness', () => {
    test('complex patterns should have comprehensive examples', () => {
      const complexPatterns = patterns.filter(
        (p) => p.components.length > 3 || p.category === 'generative-ai',
      )
    ...
    test('patterns should have comprehensive tagging', () => {
      patterns.forEach((pattern) => {
        ...
        const hasComponentTag = pattern.components.some(
          (comp) => pattern.tags.includes(comp) || pattern.tags.includes(comp.replace('-', '')),
        )
  ...
  describe('Accessibility validation', () => {
    test('patterns should consider accessibility', () => {
      ...
      patterns.forEach((pattern) => {
        const hasAccessibilityConsideration = accessibilityKeywords.some(
          (keyword) =>
            ...
            pattern.examples.some(
              (ex) =>
                ...
                ex.codeSnippet?.toLowerCase().includes(keyword),
            ),
        )

        // For now, just check that form and interactive patterns consider accessibility
        if (
          pattern.components.includes('button') ||
