...
} from '../../src/components/data/component-demo-map'
import { demos } from '../../src/components/data/demos'
...
describe('Demo Data Validation', () => {
  ...
  describe('Cross-validation with component-demo mappings', () => {
    test('all demos should exist in component-demo mapping', () => {
      const mappedDemoIds = Object.values(componentDemoMap).flatMap((mapping) => mapping.demoIds)
  ...
  describe('Accessibility validation', () => {
    test('demos should include accessibility considerations', () => {
      ...
      demos.forEach((demo) => {
        const hasAccessibilityFeatures = accessibilityKeywords.some(
          (keyword) =>
            demo.code.includes(keyword) || demo.variations.some((v) => v.code.includes(keyword)),
        )

        // For now, just ensure we're aware of accessibility - not all demos need it
        if (
          demo.componentId === 'button' ||
          demo.componentId === 'input' ||
          demo.componentId === 'form'
        ) {
