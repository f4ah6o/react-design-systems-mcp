...
import { type Demo, demoData } from '../components/data/demos'
...
export class DemoProvider {
  ...
  getComponentDemos(options: DemoSearchOptions): DemoSearchResult {
    ...
    const componentDemos = Object.values(this.demos).filter(
      (demo) => demo.componentId === componentId,
    )
    ...
    if (options.complexity) {
      filteredDemos = filteredDemos.filter(
        (demo) => demo.metadata.complexity === options.complexity,
      )
    }
  ...
  getAllDemos(options: DemoSearchOptions = {}): Demo[] {
    ...
    if (options.tags && options.tags.length > 0) {
      demos = demos.filter((demo) => options.tags?.some((tag) => demo.tags.includes(tag)))
    }

    if (options.complexity) {
      demos = demos.filter((demo) => demo.metadata.complexity === options.complexity)
    }
