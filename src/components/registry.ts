...
import demoProvider, { type DemoSearchOptions, type DemoSearchResult } from '../demo-provider'
import patternProvider, {
  type PatternDetailsOptions,
  type PatternSearchOptions,
  type PatternSearchResult,
} from '../pattern-provider'
import {
  componentHasDemos,
  getDemosForComponent,
  getPrimaryDemoForComponent,
} from './data/component-demo-map'
...
function getComponentDirectories(): string[] {
  ...
  return fs
    .readdirSync(componentsDataDir)
    .filter((item) => {
      const itemPath = path.join(componentsDataDir, item)
      return fs.statSync(itemPath).isDirectory() && fs.existsSync(path.join(itemPath, 'api.json')) // Must have api.json
    })
    .map((dir) => dir)
}
...
export function searchProperties(options: {
  ...
  const componentsToSearch = componentId ? { [componentId]: components[componentId] } : components
...
export function getComponentDemos(
  componentId: string,
  options: Omit<DemoSearchOptions, 'componentId'> = {},
): DemoSearchResult {
