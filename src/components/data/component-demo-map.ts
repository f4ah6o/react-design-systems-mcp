...
export function getComponentDemoStats() {
  ...
  const componentsWithMultipleDemos = Object.values(componentDemoMap).filter(
    (mapping) => mapping.demoCount > 1,
  ).length
...
export function validateComponentDemoMappings(): { valid: boolean; errors: string[] } {
  ...
  Object.entries(componentDemoMap).forEach(([componentId, mapping]) => {
    if (mapping.componentId !== componentId) {
      errors.push(
        `Component mapping key '${componentId}' doesn't match mapping.componentId '${mapping.componentId}'`,
      )
    }

    if (mapping.demoCount !== mapping.demoIds.length) {
      errors.push(
        `Component '${componentId}' demoCount (${mapping.demoCount}) doesn't match demoIds length (${mapping.demoIds.length})`,
      )
    }

    if (mapping.primaryDemo && !mapping.demoIds.includes(mapping.primaryDemo)) {
      errors.push(
        `Component '${componentId}' primaryDemo '${mapping.primaryDemo}' not found in demoIds`,
      )
    }
  })
  ...
  Object.entries(demoToComponentMap).forEach(([demoId, componentId]) => {
    ...
    if (!componentMapping) {
      errors.push(
        `Demo '${demoId}' maps to component '${componentId}' which doesn't exist in componentDemoMap`,
      )
...
export function findSimilarDemos(demoId: string, limit: number = 3): string[] {
  ...
  const componentDemos = getDemosForComponent(componentId).filter((id) => id !== demoId)
