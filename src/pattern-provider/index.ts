...
import { type PatternCategory, patternCategories } from '../components/data/pattern-categories'
import { type Pattern, patternData } from '../components/data/patterns'
...
export class PatternProvider {
  ...
  searchPatterns(options: PatternSearchOptions = {}): PatternSearchResult {
    ...
    if (options.component) {
      patterns = patterns.filter((pattern) => pattern.components.includes(options.component!))
    }
  ...
  getPatternDetails(options: PatternDetailsOptions): Pattern {
    ...
    if (
      typeof includeExamples !== 'boolean' ||
  ...
  getPatternCategories(
    options: { includePatternCount?: boolean; includePatternList?: boolean } = {},
  ...
  getPatternsByCategory(categoryId: string): Pattern[] {
    return Object.values(this.patterns).filter((pattern) => pattern.category === categoryId)
  }
    ...
    for (const pattern of patterns) {
      ...
      for (const relatedPatternId of pattern.relatedPatterns) {
        if (!this.patterns[relatedPatternId]) {
          errors.push(
            `Pattern ${pattern.id} references non-existent related pattern: ${relatedPatternId}`,
          )
        }
      }
  ...
  findSimilarPatterns(patternId: string, limit: number = 5): Pattern[] {
    ...
    const scoredPatterns = patterns.map((pattern) => {
      ...
      // Shared tags
      const sharedTags = pattern.tags.filter((tag) => targetPattern.tags.includes(tag)).length
      score += sharedTags * 2
